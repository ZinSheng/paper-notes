...
3. **Generate the summary** (when `next_step == "generate_summary"`) — follow the evidence-aware schema procedure below.
4. **Rebuild dashboard** — `build_dashboard.py`.
5. **Report** — tell the user the HTML path and that opening it in a browser gives full editing (auto-saves to localStorage; click "Sync Folder" to auto-write edits + status to `papers/<KEY>.edits.json`; Export/Import JSON as fallback in non-Chromium browsers).

## LLM summary generation procedure (evidence-aware schema v3)

When `add` (or `refresh --regenerate-summary`) signals summary generation is needed:

1. Read `references/summary_schema.md` for the schema, paper profiles, evidence rules, and final checklist.
...
3. Select one or more paper profiles (`empirical`, `theory`, `method`, `benchmark`, `dataset`, `survey`, `system`, `replication`, or `other`) and generate the schema defined in `references/summary_schema.md`. Hard rules:
   ...
   - Markdown is allowed in prose fields. Each substantial prose field must contain at least 2 and normally 2–6 selective `**bold**` spans for important terms, metrics, mechanisms, or caveats; do not bold entire sentences or paragraphs.
4. Write the JSON to `papers/<KEY>.summary.json`.
5. Read every section in `papers/<KEY>.section_text.json` and write `papers/<KEY>.sections.json`. Each section object must contain `heading`, `page`, `level`, `summary`, and `analysis`; `summary` states what the section says, while `analysis` evaluates evidence, assumptions, limitations, or relevance. Do not skip this step because the summary JSON was successfully written.
6. Validate that `sections.json` exists, contains at least one section with non-empty `summary` and `analysis`, preserves source order, has no duplicate section numbers, and places every parent before its subsections. The renderer also re-sorts numerically as a safety net, but numbering errors must be reported. If validation fails, report the exact artifact or numbering error and do not claim the paper is fully generated.
7. Re-run `build_paper_html.py --key <KEY>` then `build_dashboard.py`.

If `papers/<KEY>.edits.json` exists (browser-synced edits), it overrides the summary at render time AND its `status` field flows into the dashboard — user edits always win. Do not delete edits unless the user asks to discard them.
...
- **Markdown + LaTeX render uniformly in EVERY editable text field** — the 8 prose modules (objective, problem_landscape, approach, impact, cost, experiments_results, relevance_to_my_work, custom_notes), the section summaries/analyses, and the quote blocks. There is no special "formula module"; formatting is a property of the shared renderer, not of any one section.
